...
test -e /etc/restestolv.conf && echo "/etc/resolv.conf exists" || echo "/etc/resolv.conf does not exist"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is a regular file" || echo "/etc/resolv.conf is not regular file"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is a symbolic link" || echo "/etc/resolv.conf is not a symbolic link"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is a directory" || echo "/etc/resolv.conf is not a directory"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is readable" || echo "/etc/resolv.conf is not readable"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is writable" || echo "/etc/resolv.conf is not writable"
test -e /etc/restestolv.conf && echo "/etc/resolv.conf is executable" || echo "/etc/resolv.conf is not a executable"
...
##
[ $(($secondNumber % $firstnumber)) -eq 0 ]&& echo "The second number is a multiple of first number" || echo "The second number is not a mutiple of first number"
...
[ -n USER ] && echo "The data variable is: USER" || echo "No data in variable USER"
[ -z USER ] && echo "USER varaible exits but is empty" || echo "USER variable is not empty"
